<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Show_Key_indicators.aspx.cs" Inherits="Show_Key_indicators" %>
...
<script type="text/javascript" language="javascript">
    ...
    function InitKey() {
        ...
        if (mname == "" || mname == null) {
            ...
            if (Grade == 0) {
                var Str = "?act=initkey&mname=" + membername + "&year=" + passYear + "&year2=" + nowYear;
    ...
    function setYear() {
        ...
        var year1 = year - 1;
        var Str = "?act=getkeydatabyyear&year=" + year1 + "&year2=" + year + "&mname=" + membername;
        entity.PostStringInfo(Str, this, this.InfoCallBack);
        //var year1 = year - 1;
        //var Str = "?act=getkeydatabyyear&year=" + year1 + "&year2=" + year + "&mname=" + mname;
               // var year = year2 - 1;
               // var Str = "?act=getkeydatabyyear&year=" + year + "&mname=" + membername;
                // alert(Str);
               // entity.PostStringInfo(Str, this, this.InfoCallBack);
    ...
    function showHead() {
        ...
        if (Info[0].Isedit == 1 && Info[0].Isok == 0) {
            strHtml += '<input type="button" name="btSure" id="btSure" onclick="C_EditKey()" value="重改" style="width:50px; background-color:White; border:solid 1px lightgray;margin-left:10px;"/>' +
             '<input type="button" name="btSure" id="btSure" onclick="OkKey()" value="确定" style="width:50px; background-color:White; border:solid 1px lightgray;margin-left:10px;"/>';
        }
    ...
    '</tr>';
        ...
        for (var i = 0; i < Info.length; i++) {
               ...
               '<td id="11M' + i + '" style="text-align:right;" bgcolor="#C5BE97">' + Show_Red(Info[i].cYear_PlanSalesReal1) + '</td>' +
                '<td id="11M' + i + '" style="text-align:right;" bgcolor="#C5BE97">' + Show_Red_a(Info[i].cYear_CompletionRatioReal1) + '%</td>';
                   ...
                   if (Info[i].cMonth < nowMonth && Info[i].cYear2 == nowYear) {
                       ...
                       strHtml += '<td id="4M' + i + '" style="text-align:right;" bgcolor="#BFBFBF">' + Show_Red(Info[i].cYear_ActualSalesPlan2) + '</td>';
                   ...
                   if (Info[i].cMonth < nowMonth && Info[i].cYear2 == nowYear) {
                       ...
                       strHtml += '<td id="4M' + i + '" style="text-align:right;" bgcolor="#BFBFBF">' + Show_Red(Info[i].cYear_PlanSalesPlan2) + '</td>';
            ...
            '<td id="11M' + i + '" style="text-align:right;" bgcolor="#B6DDE8">' + Show_Red_a(PlanDifference.toFixed(1)) + '%</td>' +

            '<td id="11M' + i + '" style="text-align:center;">';
            if (PlanDifference > 0) {
        ...
        '<td id="11M' + i + '" style="text-align:right;" bgcolor="#9CB75B">' + Show_Red_a(cYear_Now.toFixed(1)) + '%</td>' +
         ...
         '<td id="11M' + i + '" style="text-align:right;" bgcolor="#9CB75B">' + Show_Red_a(NowDifference.toFixed(1)) + '%</td>' +
            ...
            strHtml += '</td>' +
                       ...
                       '<td id="10M' + i + '" style="text-align:left;">' + Info[i].Sketch + '</td>'+
       ...
       // var cYear_limit = cYear_limit.toFixed(1);
        ...
        strHtml += '<tr ><td style="text-align:center;" height="30">合计</td>' +
                   ...
                   '</tr>' +
                   '<tr ><td style="text-align:center;" height="30" colspan="2" bgcolor="#FFFF00">目标</td>' +


                   '<td style="text-align:right;"  colspan="2" bgcolor="#FFFF00">' + Show_Red_a(Info[0].cYear_target) + '%</td>' +
    ...
    function OkKey() {
        var year = document.getElementById("year").value;
        if (confirm("确定要提交关键指标追赶表吗？")) {
            var Str = "?act=okkey&year=" + year + "&mname=" + membername;
            // alert(Str);
            entity.PostStringInfo(Str, this, this.SaveCommitCallBack);
        }
    }
    ...
    function C_EditKey() {
        var year = document.getElementById("year").value;
        if (confirm("确定要重新修改关键指标追赶表吗？")) {
            var Str = "?act=c_editkey&year=" + year + "&mname=" + membername;
            // alert(Str);
            entity.PostStringInfo(Str, this, this.SaveCommitCallBack);
        }
    }
...
<div class="wrapper" style="height: auto; min-height: 100%;">
 ...
 <div class="content-wrapper" id="Div1" style="margin-left:15px">
  ...
  <div style="overflow-x:scroll"><table valign="top" style="border-collapse:collapse;width:99%;background:white;height:20px" border="1";>
   ...
   <td valign="top" style="width:700px;border-style:none;padding:4px;height:30px;">
